...
def handler(event: Dict, context: Dict) -> Dict:
    ...
    interest = filter(lambda x: x.get("currency") in TARGET_BALANCE.keys(), balances.get("payload").get("balances"))
    print(json.dumps(list(interest), indent=4))
    return {"status": 200}


if __name__ == '__main__':
    print(json.dumps(TARGET_BALANCE))
    handler({}, {})
